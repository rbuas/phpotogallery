...
Synapse = (function ($, tracer) {
    ...
    Synapse.prototype = {
        ...
        load : function(filename) {
            tracer.comm(this.options.name + "::loadfile: ", filename, true);
            var _this = this;
            return $.ajax({
                url: filename,
                contentType: "application/json; charset=utf-8",
        ...
        connect : function(slug, ctype) {
            var synapse  = this.getUrl("connect");
            if(!synapse)
                return false;

            var params = [];
            if(slug) params.push("slug=" + slug);
            if(ctype) params.push("ctype=" + ctype);

            var paramsuri = params.length > 0 ? "?" + params.join("&") : "";
            return this.get(synapse + paramsuri);
        },
        ...
        memoGet : function(memo) {
            var synapse  = this.getUrl("memoGet");
            if(!synapse)
                return false;

            return this.post(synapse, {memo:memo});
        },

        memoSave : function(memo, memodata) {
            var synapse  = this.getUrl("memoSave");
            if(!synapse)
                return false;

            return this.post(synapse, {memo:memo, memodata:memodata});
        },

        memoRemove : function(memo) {
            var synapse  = this.getUrl("memoRemove");
            if(!synapse)
                return false;
        ...
        memoList : function(callback) {
            var synapse  = this.getUrl("memoList");
            if(!synapse)
                return false;
