<?php
class Brain extends Synapse {

    public function __construct ( $config, $map) {
        $this->process = [];
   ...
   ///////
    ...
    private function LoadConfigFiles () {
        $this->config = [];
        if($this->configFile) {
            $serverfile = ServerGetFile($this->configFile);
            if(file_exists($serverfile)) {
                $configContent = FileRead( $serverfile );
                if($configContent)
        ...
        $this->sitemap = [];
        if($this->mapFile) {
            $serverfile = ServerGetFile($this->mapFile);
            if( file_exists($serverfile) ) {
                $mapContent = FileRead( $serverfile );
                if($mapContent)
